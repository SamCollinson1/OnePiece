...
function chooseCharacter(){
    ...
    chosenCharacterAge = chosenCharacter[characterName][3];
    chosenCharacterHeight = chosenCharacter[characterName][5];
    chosenCharacterBounty = chosenCharacter[characterName][4];
}
    ...
    if (!addedCharacters.includes(inputValue) && checkCharacter(inputValue)) {
        ...
        if (selectedCharacter) {
            ...
            cellData.forEach((data, index) => {
                ...
                console.log(chosenCharacter[Object.keys(chosenCharacter)[0]][index-1]);
                // Check if the detail matches with the chosenCharacter
                if (chosenCharacter) {
                    if (details[index - 1] === chosenCharacter[Object.keys(chosenCharacter)[0]][index-1]) {
                    ...
                    if (index === 4 || index === 6 || index === 5) {
                        // Age
                        if (index === 4) {
                            if (chosenCharacterAge < details[3]) {
                                addCell.classList.add("downarrow");
                            } else if (chosenCharacterAge > details[3]) {
                                addCell.classList.add("uparrow");
                            }
                        }
                        // Bounty
                        if (index === 5) {
                            console.log(compareBounties(chosenCharacterBounty, details[4]));
                            if (compareBounties(chosenCharacterBounty, details[4]) === "selected") {
                                addCell.classList.add('downarrow');
                            } else if (compareBounties(chosenCharacterBounty, details[4]) === "chosen") {
                                addCell.classList.add('uparrow');
                            }
                        }
                        // Height
                        if (index === 6) {
                            if (chosenCharacterHeight < details[5]) {
                                addCell.classList.add("downarrow");
                            } else if (chosenCharacterHeight > details[5]) {
                                addCell.classList.add("uparrow");
                            }
                        }
                    }
